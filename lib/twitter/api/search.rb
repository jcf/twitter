...
module Nunemaker::Twitter
  module API
    module Search
      ...
      # @note Please note that Nunemaker::Twitter's search service and, by extension, the Search API is not meant to be an exhaustive source of Tweets. Not all Tweets will be indexed or made available via the search interface.
      # @rate_limited Yes
      ...
      # @option options [String] :geocode Returns tweets by users located within a given radius of the given latitude/longitude. The location is preferentially taking from the Geotagging API, but will fall back to their Nunemaker::Twitter profile. The parameter value is specified by "latitude,longitude,radius", where radius units must be specified as either "mi" (miles) or "km" (kilometers). Note that you cannot use the near operator via the API to geocode arbitrary locations; however you can use this geocode parameter to search near geocodes directly.
      ...
      # @return [Nunemaker::Twitter::SearchResults] Return tweets that match a specified query with search metadata
      # @example Return tweets related to twitter
      #   Nunemaker::Twitter.search('twitter')
      def search(q, options={})
        object_from_response(Nunemaker::Twitter::SearchResults, :get, "/1.1/search/tweets.json", options.merge(:q => q))
      end
