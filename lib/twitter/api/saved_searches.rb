...
module Nunemaker::Twitter
  module API
    module SavedSearches
      ...
      # @return [Array<Nunemaker::Twitter::SavedSearch>] The saved searches.
      ...
      #     Nunemaker::Twitter.saved_searches
      ...
      #     Nunemaker::Twitter.saved_search(16129012)
      ...
      def saved_searches(*args)
        ...
        if arguments.empty?
          objects_from_response(Nunemaker::Twitter::SavedSearch, :get, "/1.1/saved_searches/list.json", arguments.options)
        else
      ...
      # @authentication Requires user context
      # @raise [Nunemaker::Twitter::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Nunemaker::Twitter::SavedSearch] The saved searches.
      # @param id [Integer] The ID of the saved search.
      # @param options [Hash] A customizable set of options.
      # @example Retrieve the data for a saved search owned by the authenticating user with the ID 16129012
      #   Nunemaker::Twitter.saved_search(16129012)
      def saved_search(id, options={})
        object_from_response(Nunemaker::Twitter::SavedSearch, :get, "/1.1/saved_searches/show/#{id}.json", options)
      end
      ...
      # @authentication Requires user context
      # @raise [Nunemaker::Twitter::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Nunemaker::Twitter::SavedSearch] The created saved search.
      # @param query [String] The query of the search the user would like to save.
      # @param options [Hash] A customizable set of options.
      # @example Create a saved search for the authenticated user with the query "twitter"
      #   Nunemaker::Twitter.saved_search_create("twitter")
      def saved_search_create(query, options={})
        object_from_response(Nunemaker::Twitter::SavedSearch, :post, "/1.1/saved_searches/create.json", options.merge(:query => query))
      end
      ...
      # @authentication Requires user context
      # @raise [Nunemaker::Twitter::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<Nunemaker::Twitter::SavedSearch>] The deleted saved searches.
      # @overload saved_search_destroy(*ids)
      #   @param ids [Enumerable<Integer>] A collection of saved search IDs.
      #   @example Destroys a saved search for the authenticated user with the ID 16129012
      #     Nunemaker::Twitter.saved_search_destroy(16129012)
      ...
      def saved_search_destroy(*args)
        arguments = Nunemaker::Twitter::API::Arguments.new(args)
        arguments.flatten.threaded_map do |id|
          object_from_response(Nunemaker::Twitter::SavedSearch, :post, "/1.1/saved_searches/destroy/#{id}.json", arguments.options)
        end
      end
