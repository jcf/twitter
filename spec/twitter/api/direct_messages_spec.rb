...
describe Nunemaker::Twitter::API::DirectMessages do
  ...
  describe "#direct_messages_received" do
    ...
    it "returns the 20 most recent direct messages sent to the authenticating user" do
      ...
      expect(direct_messages.first).to be_a Nunemaker::Twitter::DirectMessage
  ...
  describe "#direct_message" do
    ...
    it "returns the specified direct message" do
      ...
      expect(direct_message).to be_a Nunemaker::Twitter::DirectMessage
  ...
  describe "#direct_message_create" do
    before do
      stub_post("/1.1/direct_messages/new.json").with(:body => {:screen_name => "pengwynn", :text => "Creating a fixture for the Nunemaker::Twitter gem"}).to_return(:body => fixture("direct_message.json"), :headers => {:content_type => "application/json; charset=utf-8"})
    end
    it "requests the correct resource" do
      @client.direct_message_create("pengwynn", "Creating a fixture for the Nunemaker::Twitter gem")
      expect(a_post("/1.1/direct_messages/new.json").with(:body => {:screen_name => "pengwynn", :text => "Creating a fixture for the Nunemaker::Twitter gem"})).to have_been_made
    end
    it "returns the sent message" do
      direct_message = @client.direct_message_create("pengwynn", "Creating a fixture for the Nunemaker::Twitter gem")
      expect(direct_message).to be_a Nunemaker::Twitter::DirectMessage
      expect(direct_message.text).to eq "Creating a fixture for the Nunemaker::Twitter gem"
    end
    context "with a URI object passed" do
      it "requests the correct resource" do
        user = URI.parse("https://twitter.com/pengwynn")
        @client.direct_message_create(user, "Creating a fixture for the Nunemaker::Twitter gem")
        expect(a_post("/1.1/direct_messages/new.json").with(:body => {:screen_name => "pengwynn", :text => "Creating a fixture for the Nunemaker::Twitter gem"})).to have_been_made
      end
    end
    context "with a URI string passed" do
      it "requests the correct resource" do
        @client.direct_message_create("https://twitter.com/pengwynn", "Creating a fixture for the Nunemaker::Twitter gem")
        expect(a_post("/1.1/direct_messages/new.json").with(:body => {:screen_name => "pengwynn", :text => "Creating a fixture for the Nunemaker::Twitter gem"})).to have_been_made
      end
    end
  end
